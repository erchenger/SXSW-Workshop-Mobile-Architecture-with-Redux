...
class ListViewModelTests: XCTestCase {
    ...
    func testShowsActivityIndicator() {
        let state = ListState(isLoading: true)
        let viewModel = ListViewModel(state)
        XCTAssertTrue(viewModel.showsActivityIndicator)
    }
    
    func testItemViewModels() {
        let state = ListState(searchResponse: .success(.mockResponse))
        let viewModel = ListViewModel(state)
        XCTAssertEqual(viewModel.itemViewModels.count, SearchResponse.mockResponse.results.count)
    }
}
