...
class ListViewControllerTests: XCTestCase {
    ...
    func testTypingDispatchesAction() {
        ...
        XCTAssertTrue(mockStore.dispatchedActions?[0] is ListAction.SetSearchText)
        XCTAssertTrue(mockStore.dispatchedActions?[1] is ListAction.ExecuteSearch)
    }
    
    func testReturnKeyTriggersSearch() {
        _ = viewController.textFieldShouldReturn(viewController.textField)
        XCTAssertTrue(mockStore.lastAction is ListAction.ExecuteSearch)
    }
    
    func testSearchButtonTriggersSearch() {
        viewController.submitPressed()
        XCTAssertTrue(mockStore.lastAction is ListAction.ExecuteSearch)
    }
}
